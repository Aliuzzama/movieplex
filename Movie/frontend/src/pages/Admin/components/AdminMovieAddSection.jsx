...
export const AdminMovieAddSection = () => {
  ...
  const isFormValid = () => {
    return (
      ...
      !isNaN(movieInfo.rating) &&
      movieInfo.duration &&
      movieInfo.cast &&
      movieInfo.relDate &&
      Array.isArray(movieInfo.genres) &&
      movieInfo.genres.filter((genre) => genre.trim() !== "").length > 0 &&
      Array.isArray(movieInfo.directors) &&
      movieInfo.directors.filter((director) => director.trim() !== "").length > 0
    );
  };
        ...
        <form className="form-movie-add" onSubmit={movieAdd}>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => handleMovieInfo(e)}
              type="text"
              placeholder="Enter Movie Duration (Format: *H **M)"
            />
          </div>
          ...
          <button
  type="submit"
  className={`btn-admin ${!isFormValid() ? "btn-disabled" : ""}`}
