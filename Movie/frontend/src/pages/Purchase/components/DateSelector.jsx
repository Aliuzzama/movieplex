...
export const DateSelector = ({ paymentOngoing }) => {
  ...
  // Extract unique dates from showDatesData
  const uniqueShowDates = Array.from(
    new Set(showDatesData.map((dateData) => {
      const date = new Date(dateData.showtime_date);
      return date.toISOString().split('T')[0]; // Get the date in YYYY-MM-DD format
    }))
  ).map((date) => {
    return showDatesData.find((dateData) => new Date(dateData.showtime_date).toISOString().split('T')[0] === date);
  });
  ...
  const dateOptions = uniqueShowDates.map((dateData, idx) => {
